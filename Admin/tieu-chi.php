...
$tieuchis = $UserModel->getAllTieuchi();
?>
                  ...
                  <tr>
                    ...
                    <td class="align-middle">
                      <a
                        ...
                        <img
                          ...
                          src="./thumb/<?php echo $tieuchi['photo'] ?>"
                    ...
                    <td class="align-middle text-center">
                      ...
                      <input
                          ...
                          type="checkbox"
                          data-loai="noibat"
                          btn-id="btn-noibat-<?= $tieuchi['tieuchi_id'] ?>"
                          data-id-noibat="<?= $tieuchi['tieuchi_id'] ?>"
                          <?= $tieuchi['noibat'] == 1 ? "checked" : "" ?>
                        />
                      </div>
                    </td>
                    <td class="align-middle text-center">
                      <div class="custom-control custom-checkbox my-checkbox">
                      <input
                          btn-id="btn-hienthi-<?= $tieuchi['tieuchi_id'] ?>"
                          data-id = "<?= $tieuchi['tieuchi_id'] ?>"
                          type="checkbox"
                          data-loai="hienthi"
                          <?= $tieuchi['hienthi'] == 1 ? "checked" : "" ?>
                        />
                      </div>
                    </td>
                    <td class="align-middle text-center text-md text-nowrap">
                      <a
                        ...
                        href="add_tieuchi.php?tieuchi_id=<?= md5($tieuchi['tieuchi_id'] . "maixep") ?>"
                        title="Chỉnh sửa"
                        ><i class="bi bi-pencil-square"></i></a>
                      <a
                        class="text-danger"
                        id="delete-item"
                        href="delete_tieuchi.php?tieuchi_id=<?=  md5($tieuchi['tieuchi_id'] . "maixep") ?>"
                        title="Xóa"
    ...
    $(document).ready(function(){
      $("[data-id]").click(function(){
          ...
          if ($(this).is(':checked')) {
            ...
            // console.log(id);
            $.ajax({
              url: "dmc1_action_hienthi.php?atc=tieuchi",
              method: "POST",
              data:{id:id},
          ...
          var id = $(this).attr('data-id');
            // console.log(id);
            $.ajax({
              url: "dmc1_deletecheck_hienthi.php?atc=tieuchi",
              method: "POST",
              data:{id:id},
              success:function(data){
                if(data == 1){
        ...
        $("[data-id-noibat]").click(function(){
          ...
          if ($(this).is(':checked')) {
            ...
            console.log(id);
            $.ajax({
              url: "dmc1_action.php?atc=tieuchi",
              method: "POST",
              data:{id:id},
              success:function(data){
                if(data = 1){
          ...
          // var id = $(this).attr('data-id');
            console.log(id);
            $.ajax({
              url: "dmc1_deletecheck.php?atc=tieuchi",
              method: "POST",
              data:{id:id},
              success:function(data){
                if(data == 1){
