  ...
  if(!empty($_POST['name']) && !empty($_POST['link']) && !empty($_FILES['fileupload'])) {
    ...
    if (!in_array($imageFileType,$allowtypes ))
    {
      ...
      if($_FILES['fileupload']['tmp_name'] == ''){
        ...
        if(isset($_GET['id'])) {
          // echo(1);die();
          $UserModel->insertMangxhTop($_GET['id'],$name, $link, $photo);
          header('Location: mang-xh-top.php');
      }
      else{
        ...
        if(is_uploaded_file($_FILES['fileupload']['tmp_name']) && move_uploaded_file($_FILES['fileupload']['tmp_name'], $foderPath)){
          ...
          if(isset($_GET['id'])) {
              // echo(1);die();
              $UserModel->updaytMangxhTop($_GET['id'],$name, $link, $photo);
              header('Location: mang-xh-top.php');
          }
          else{
            // echo(1);die();
              $UserModel->insertMangxhTop($name, $link, $photo);
              header('Location: mang-xh-top.php');
          }
          }
        else{
