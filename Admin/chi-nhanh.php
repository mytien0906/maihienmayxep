                  ...
                  <tr>
                    ...
                    <td class="align-middle text-center">
                      ...
                      <input
                          ...
                          type="checkbox"
                          data-loai="noibat"
                          btn-id="btn-noibat-<?= $chinhanh['chinhanh_id'] ?>"
                          data-id-noibat="<?= $chinhanh['chinhanh_id'] ?>"
                          <?= $chinhanh['noibat'] == 1 ? "checked" : "" ?>
                        />
                      </div>
                    </td>
                    <td class="align-middle text-center">
                      <div class="custom-control custom-checkbox my-checkbox">
                      <input
                          btn-id="btn-hienthi-<?= $chinhanh['chinhanh_id'] ?>"
                          data-id = "<?= $chinhanh['chinhanh_id'] ?>"
                          type="checkbox"
                          data-loai="hienthi"
                          <?= $chinhanh['hienthi'] == 1 ? "checked" : "" ?>
                        />
                      </div>
                    </td>
                    <td class="align-middle text-center text-md text-nowrap">
                      <a
                        ...
                        href="add_chinhanh.php?chinhanh_id=<?= md5($chinhanh['chinhanh_id'] . "maixep") ?>"
                        title="Chỉnh sửa"
                        ><i class="bi bi-pencil-square"></i></a>
                      <a
                        class="text-danger"
                        id="delete-item"
                        href="delete_chinhanh.php?chinhanh_id=<?= md5($chinhanh['chinhanh_id'] . "maixep") ?>"
                        title="Xóa"
...
$(document).ready(function(){
  $("[data-id]").click(function(){
      ...
      if ($(this).is(':checked')) {
        ...
        // console.log(id);
        $.ajax({
          url: "dmc1_action_hienthi.php?atc=chinhanh",
          method: "POST",
          data:{id:id},
      ...
      var id = $(this).attr('data-id');
        // console.log(id);
        $.ajax({
          url: "dmc1_deletecheck_hienthi.php?atc=chinhanh",
          method: "POST",
          data:{id:id},
          success:function(data){
            if(data == 1){
    ...
    $("[data-id-noibat]").click(function(){
      ...
      if ($(this).is(':checked')) {
        ...
        console.log(id);
        $.ajax({
          url: "dmc1_action.php?atc=chinhanh",
          method: "POST",
          data:{id:id},
          success:function(data){
            if(data = 1){
      ...
      // var id = $(this).attr('data-id');
        console.log(id);
        $.ajax({
          url: "dmc1_deletecheck.php?atc=chinhanh",
          method: "POST",
          data:{id:id},
          success:function(data){
            if(data == 1){
